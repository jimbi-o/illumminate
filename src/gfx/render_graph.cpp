...
static auto FindConcurrentPass(const uint32_t pass_num, const unordered_map<uint32_t, unordered_map<uint32_t, int32_t>> node_distance_map, std::pmr::memory_resource* memory_resource) {
  ...
  for (uint32_t src = 0; src < pass_num; src++) {
    ...
    auto& distance_map = node_distance_map.at(src);
  ...
  for (uint32_t pass_index = 1; pass_index < pass_num; pass_index++) {
    ...
    for (auto& expired_buffer : render_pass_expired_buffer_list[pass_index-1]) {
      ...
      for (auto it = memory_reusable_buffer_list.begin(); it != memory_reusable_buffer_list.end(); it++) {
        auto& comp_buffer = *it;
        auto& comp_buffer_addr = buffer_address_offset_list.at(comp_buffer);
    ...
    for (auto& new_buffer : render_pass_new_buffer_list[pass_index]) {
      if (excluded_buffers.contains(new_buffer)) continue;
      auto& buffer_config = buffer_config_list.at(new_buffer);
      for (auto it = memory_reusable_buffer_list.begin(); it != memory_reusable_buffer_list.end(); it++) {
    ...
    for (auto& buffer_id : concurrent_buffer_list[pass_index]) {
...
TEST_CASE("buffer reuse") {
  ...
  unordered_map<BufferId, unordered_map<BufferId, int32_t>> node_distance_map;
  {
    ...
    CHECK(concurrent_pass_list.size() == 4);
    CHECK(concurrent_pass_list[0].empty());
    CHECK(concurrent_pass_list[1].empty());
    CHECK(concurrent_pass_list[2].empty());
    CHECK(concurrent_pass_list[3].empty());
...
TEST_CASE("memory aliasing") {
  ...
  node_distance_map = AddNodeDistanceInReverseOrder(std::move(node_distance_map));
  auto concurrent_pass_list = FindConcurrentPass(static_cast<uint32_t>(render_pass_command_queue_type_list.size()), node_distance_map, &memory_resource_work);
  CHECK(concurrent_pass_list.size() == 5);
  CHECK(concurrent_pass_list[0].empty());
  CHECK(concurrent_pass_list[1].empty());
  CHECK(concurrent_pass_list[2].empty());
  CHECK(concurrent_pass_list[3].empty());
  CHECK(concurrent_pass_list[4].empty());
  auto concurrent_buffer_list = FindConcurrentBuffers(concurrent_pass_list, render_pass_buffer_id_list, &memory_resource_work);
  ...
  CHECK(render_pass_before_memory_aliasing_list.at(4)[0] == 1);
  CHECK(render_pass_after_memory_aliasing_list.size() == 1);
  CHECK(!render_pass_after_memory_aliasing_list.contains(0));
  CHECK(!render_pass_after_memory_aliasing_list.contains(1));
  CHECK(!render_pass_after_memory_aliasing_list.contains(2));
  CHECK(!render_pass_after_memory_aliasing_list.contains(3));
  CHECK(render_pass_after_memory_aliasing_list.contains(4));
  CHECK(render_pass_after_memory_aliasing_list.at(4)[0] == 4);
}
TEST_CASE("ConfigureBufferValidPassList w/async compute") {
  ...
  unordered_map<BufferId, uint32_t> buffer_size_list{&memory_resource_work};
  buffer_size_list.insert_or_assign(0, 4);
  buffer_size_list.insert_or_assign(1, 4);
  buffer_size_list.insert_or_assign(2, 4);
  buffer_size_list.insert_or_assign(3, 4);
  buffer_size_list.insert_or_assign(4, 4);
  buffer_size_list.insert_or_assign(5, 4);
  buffer_size_list.insert_or_assign(6, 4);
  buffer_size_list.insert_or_assign(7, 4);
  buffer_size_list.insert_or_assign(8, 4);
  unordered_map<BufferId, uint32_t> buffer_alignment_list{&memory_resource_work};
  buffer_alignment_list.insert_or_assign(0, 8);
  buffer_alignment_list.insert_or_assign(1, 8);
  buffer_alignment_list.insert_or_assign(2, 8);
  ...
  auto concurrent_pass_list = FindConcurrentPass(static_cast<uint32_t>(render_pass_command_queue_type_list.size()), node_distance_map, &memory_resource_work);
  CHECK(concurrent_pass_list.size() == 7);
  CHECK(concurrent_pass_list[0].size() == 4);
  CHECK(concurrent_pass_list[0][0] == 1);
  CHECK(concurrent_pass_list[0][1] == 3);
  CHECK(concurrent_pass_list[0][2] == 4);
  CHECK(concurrent_pass_list[0][3] == 5);
  CHECK(concurrent_pass_list[1].size() == 1);
  CHECK(concurrent_pass_list[1][0] == 0);
  CHECK(concurrent_pass_list[2].size() == 3);
  CHECK(concurrent_pass_list[2][0] == 3);
  CHECK(concurrent_pass_list[2][1] == 4);
  CHECK(concurrent_pass_list[2][2] == 5);
  CHECK(concurrent_pass_list[3].size() == 2);
  CHECK(concurrent_pass_list[3][0] == 0);
  CHECK(concurrent_pass_list[3][1] == 2);
  CHECK(concurrent_pass_list[4].size() == 2);
  CHECK(concurrent_pass_list[4][0] == 0);
  CHECK(concurrent_pass_list[4][1] == 2);
  CHECK(concurrent_pass_list[5].size() == 2);
  CHECK(concurrent_pass_list[5][0] == 0);
  CHECK(concurrent_pass_list[5][1] == 2);
  CHECK(concurrent_pass_list[6].empty());
  auto concurrent_buffer_list = FindConcurrentBuffers(concurrent_pass_list, render_pass_buffer_id_list, &memory_resource_work);
  CHECK(concurrent_buffer_list.size() == 7);
  SUBCASE("reuse only") {
    auto [buffer_address_offset_list, renamed_buffers, render_pass_before_memory_aliasing_list, render_pass_after_memory_aliasing_list] = ConfigureBufferAddressOffset(static_cast<uint32_t>(render_pass_command_queue_type_list.size()), concurrent_buffer_list, render_pass_new_buffer_list, render_pass_expired_buffer_list, buffer_config_list, buffer_size_list, buffer_alignment_list, {}, &memory_resource_work, &memory_resource_work);
  ...
  SUBCASE("with memory aliasing") {
    buffer_size_list.at(2) = 8;
    buffer_config_list.at(2).width = 123;
    ...
    CHECK(buffer_address_offset_list.at(1) == 8);
    CHECK(buffer_address_offset_list.at(2) == 0);
    CHECK(buffer_address_offset_list.at(3) == 24);
    CHECK(buffer_address_offset_list.at(4) == 16);
    CHECK(buffer_address_offset_list.at(5) == 24);
    CHECK(buffer_address_offset_list.at(6) == 8);
    CHECK(buffer_address_offset_list.at(7) == 16);
    CHECK(buffer_address_offset_list.at(8) == 16);
    CHECK(renamed_buffers.size() == 3);
    CHECK(renamed_buffers.at(6) == 1);
    CHECK(renamed_buffers.at(8) == 7);
    CHECK(renamed_buffers.at(4) == 8);
    CHECK(render_pass_before_memory_aliasing_list.size() == 2);
    CHECK(render_pass_after_memory_aliasing_list.size() == 2);
    CHECK(!render_pass_before_memory_aliasing_list.contains(0));
    CHECK(!render_pass_before_memory_aliasing_list.contains(1));
    CHECK(render_pass_before_memory_aliasing_list.contains(2));
    CHECK(!render_pass_before_memory_aliasing_list.contains(3));
    CHECK(!render_pass_before_memory_aliasing_list.contains(4));
    CHECK(render_pass_before_memory_aliasing_list.contains(5));
    CHECK(!render_pass_before_memory_aliasing_list.contains(6));
    CHECK(render_pass_before_memory_aliasing_list[2].size() == 1);
    CHECK(render_pass_before_memory_aliasing_list[2][0] == 0);
    CHECK(render_pass_after_memory_aliasing_list[2].size() == 1);
    CHECK(render_pass_after_memory_aliasing_list[2][0] == 2);
    CHECK(render_pass_before_memory_aliasing_list[5].size() == 1);
    CHECK(render_pass_before_memory_aliasing_list[5][0] == 3);
    CHECK(render_pass_after_memory_aliasing_list[5].size() == 1);
    CHECK(render_pass_after_memory_aliasing_list[5][0] == 5);
    renamed_buffers = MergeRenamedBufferDuplicativeBufferIds(buffer_id_list, std::move(renamed_buffers));
    CHECK(renamed_buffers.size() == 3);
    CHECK(renamed_buffers.at(6) == 1);
    CHECK(renamed_buffers.at(8) == 7);
    CHECK(renamed_buffers.at(4) == 7);
    buffer_id_list = UpdateBufferIdListUsingReusableBuffers(renamed_buffers, std::move(buffer_id_list));
    CHECK(buffer_id_list.size() == 6);
    CHECK(buffer_id_list[0] == 0);
    CHECK(buffer_id_list[1] == 1);
    CHECK(buffer_id_list[2] == 7);
    CHECK(buffer_id_list[3] == 2);
    CHECK(buffer_id_list[4] == 3);
    CHECK(buffer_id_list[5] == 5);
    render_pass_buffer_id_list = UpdateRenderPassBufferIdListUsingReusableBuffers(renamed_buffers, std::move(render_pass_buffer_id_list));
    CHECK(render_pass_buffer_id_list[0][0] == 0);
    CHECK(render_pass_buffer_id_list[1][0] == 1);
    CHECK(render_pass_buffer_id_list[1][1] == 7);
    CHECK(render_pass_buffer_id_list[2][0] == 1);
    CHECK(render_pass_buffer_id_list[2][1] == 2);
    CHECK(render_pass_buffer_id_list[3][0] == 3);
    CHECK(render_pass_buffer_id_list[3][1] == 7);
    CHECK(render_pass_buffer_id_list[4][0] == 3);
    CHECK(render_pass_buffer_id_list[4][1] == 7);
    CHECK(render_pass_buffer_id_list[5][0] == 7);
    CHECK(render_pass_buffer_id_list[5][1] == 5);
    CHECK(render_pass_buffer_id_list[6][0] == 5);
    CHECK(render_pass_buffer_id_list[6][1] == 1);
    auto buffer_state_list = CreateBufferStateListBasedOnBufferUserListForTest(buffer_user_pass_list, &memory_resource_work);
    std::tie(buffer_state_list, buffer_user_pass_list) = UpdateBufferStateListAndUserPassListUsingReusableBuffers(renamed_buffers, std::move(buffer_state_list), std::move(buffer_user_pass_list));
    CHECK(buffer_state_list.size() == 6);
    CHECK(!buffer_state_list.contains(4));
    CHECK(!buffer_state_list.contains(6));
    CHECK(!buffer_state_list.contains(8));
    CHECK(buffer_state_list.at(0).size() == 1);
    CHECK(buffer_state_list.at(0)[0] == kBufferStateFlagCopyDst);
    CHECK(buffer_state_list.at(1).size() == 3);
    CHECK(buffer_state_list.at(1)[0] == kBufferStateFlagCopyDst);
    CHECK(buffer_state_list.at(1)[1] == kBufferStateFlagCopySrc);
    CHECK(buffer_state_list.at(1)[2] == kBufferStateFlagCopyDst);
    CHECK(buffer_state_list.at(2).size() == 1);
    CHECK(buffer_state_list.at(2)[0] == kBufferStateFlagCopyDst);
    CHECK(buffer_state_list.at(3).size() == 2);
    CHECK(buffer_state_list.at(3)[0] == kBufferStateFlagCopyDst);
    CHECK(buffer_state_list.at(3)[1] == kBufferStateFlagCopySrc);
    CHECK(buffer_state_list.at(5).size() == 2);
    CHECK(buffer_state_list.at(5)[0] == kBufferStateFlagCopyDst);
    CHECK(buffer_state_list.at(5)[1] == kBufferStateFlagCopySrc);
    CHECK(buffer_state_list.at(7).size() == 4);
    CHECK(buffer_state_list.at(7)[0] == kBufferStateFlagCopyDst);
    CHECK(buffer_state_list.at(7)[1] == kBufferStateFlagCopyDst);
    CHECK(buffer_state_list.at(7)[2] == kBufferStateFlagCopyDst);
    CHECK(buffer_state_list.at(7)[3] == kBufferStateFlagCopySrc);
    CHECK(buffer_user_pass_list.size() == 6);
    CHECK(!buffer_user_pass_list.contains(4));
    CHECK(!buffer_user_pass_list.contains(6));
    CHECK(!buffer_user_pass_list.contains(8));
    CHECK(buffer_user_pass_list.at(0).size() == 1);
    CHECK(buffer_user_pass_list.at(0)[0][0] == 0);
    CHECK(buffer_user_pass_list.at(1).size() == 3);
    CHECK(buffer_user_pass_list.at(1)[0][0] == 1);
    CHECK(buffer_user_pass_list.at(1)[1][0] == 2);
    CHECK(buffer_user_pass_list.at(1)[2][0] == 6);
    CHECK(buffer_user_pass_list.at(2).size() == 1);
    CHECK(buffer_user_pass_list.at(2)[0][0] == 2);
    CHECK(buffer_user_pass_list.at(3).size() == 2);
    CHECK(buffer_user_pass_list.at(3)[0][0] == 3);
    CHECK(buffer_user_pass_list.at(3)[1][0] == 4);
    CHECK(buffer_user_pass_list.at(5).size() == 2);
    CHECK(buffer_user_pass_list.at(5)[0][0] == 5);
    CHECK(buffer_user_pass_list.at(5)[1][0] == 6);
    CHECK(buffer_user_pass_list.at(7).size() == 4);
    CHECK(buffer_user_pass_list.at(7)[0][0] == 1);
    CHECK(buffer_user_pass_list.at(7)[1][0] == 3);
    CHECK(buffer_user_pass_list.at(7)[2][0] == 4);
    CHECK(buffer_user_pass_list.at(7)[3][0] == 5);
    ...
    buffer_config_list = MergeReusedBufferConfigs(renamed_buffers, std::move(buffer_config_list));
    CHECK(buffer_config_list.size() == 6);
    CHECK(buffer_config_list.contains(0));
    CHECK(buffer_config_list.contains(1));
    CHECK(buffer_config_list.contains(2));
    CHECK(buffer_config_list.contains(3));

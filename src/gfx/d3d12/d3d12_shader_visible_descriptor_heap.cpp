...
namespace {
void CopyToImpl(D3d12Device* const device, const D3D12_CPU_DESCRIPTOR_HANDLE src, const uint32_t copy_num, ID3D12DescriptorHeap* const dst_heap, const D3D12_DESCRIPTOR_HEAP_TYPE type, uint32_t * const used_num, const uint32_t max_num, const uint32_t handle_increment_size) {
  if (*used_num + copy_num <= max_num) {
    D3D12_CPU_DESCRIPTOR_HANDLE dst = {dst_heap->GetCPUDescriptorHandleForHeapStart().ptr + handle_increment_size * *used_num};
    device->CopyDescriptorsSimple(copy_num, dst, src, type);
    *used_num += copy_num;
    return;
  }
  auto heap_start = dst_heap->GetCPUDescriptorHandleForHeapStart().ptr;
...
void ShaderVisibleDescriptorHeap::CopyToBuffer(const D3D12_CPU_DESCRIPTOR_HANDLE start, const uint32_t num) {
  CopyToImpl(device_, start, num, descriptor_heap_buffers_, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, &used_buffer_num_, kMaxBufferNum, buffer_handle_increment_size_);
}
void ShaderVisibleDescriptorHeap::CopyToSampler(const D3D12_CPU_DESCRIPTOR_HANDLE start, const uint32_t num) {
  CopyToImpl(device_, start, num, descriptor_heap_samplers_, D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER, &used_sampler_num_, kMaxSamplerNum, sampler_handle_increment_size_);
}

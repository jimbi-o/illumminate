...
bool CommandQueue::Init(D3d12Device* const device) {
  ...
  for (auto& type : kCommandListTypeSet) {
...
void CommandQueue::RegisterSignal(const CommandListType command_queue_type, const uint64_t val) {
...
void CommandQueue::RegisterWaitOnQueue(const CommandListType signal_queue, const uint64_t val, const CommandListType waiting_queue) {
...
void CommandQueue::WaitOnCpu(std::unordered_map<CommandListType, uint64_t>&& signals) {
...
TEST_CASE("command queue") {
  ...
  SUBCASE("cpu bound (frame bufferred)") {
    command_queue.RegisterSignal(CommandListType::kGraphics, 1);
    command_queue.WaitOnCpu({{CommandListType::kGraphics, 1},});
    command_queue.RegisterSignal(CommandListType::kGraphics, 101);
    command_queue.RegisterSignal(CommandListType::kCompute,  102);
    command_queue.WaitOnCpu({{CommandListType::kGraphics, 101}, {CommandListType::kCompute, 102},});
  }
  SUBCASE("gpu bound") { // e.g. graphics<->compute sync, wait for texture buffer transferred to GPU
    command_queue.RegisterSignal(CommandListType::kGraphics, 1);
    command_queue.RegisterSignal(CommandListType::kGraphics, 2);
    command_queue.RegisterWaitOnQueue(CommandListType::kGraphics, 1, CommandListType::kCompute);
    command_queue.RegisterSignal(CommandListType::kCompute,  3);
    command_queue.RegisterWaitOnQueue(CommandListType::kCompute, 3, CommandListType::kGraphics);
  }
  SUBCASE("single frame and multi frame mixed") {
    command_queue.RegisterSignal(CommandListType::kTransfer, 1);
    command_queue.RegisterSignal(CommandListType::kGraphics, 1);
    command_queue.RegisterSignal(CommandListType::kGraphics, 2);
    command_queue.RegisterWaitOnQueue(CommandListType::kGraphics, 1, CommandListType::kCompute);
    command_queue.RegisterSignal(CommandListType::kCompute,  3);
    command_queue.RegisterWaitOnQueue(CommandListType::kCompute,  3, CommandListType::kGraphics);
    command_queue.RegisterSignal(CommandListType::kGraphics, 3);
    command_queue.WaitOnCpu({{CommandListType::kGraphics, 3},});
    command_queue.RegisterWaitOnQueue(CommandListType::kTransfer, 1, CommandListType::kGraphics);
    command_queue.RegisterSignal(CommandListType::kGraphics, 4);
    command_queue.RegisterSignal(CommandListType::kCompute,  4);
    command_queue.WaitOnCpu({{CommandListType::kGraphics, 4}, {CommandListType::kCompute, 4}});
  }
  command_queue.Term();
  device.Term();
  dxgi_core.Term();

...
struct ParsedRenderGraphPassInfo {
  uint32_t command_list_index_prev_function;
  uint32_t command_list_index_render_function;
  uint32_t command_list_index_post_function;
  RenderPassFunction render_function;
  RenderPassResourceConfig resource_config;
};
...
ParsedRenderGraphInfo ParseRenderGraph(RenderGraphConfig&& graph_config) {
  ...
  for (auto& src_batch : graph_config.batch) {
    ...
    for (auto& src_pass : src_batch.pass) {
      ...
      dst_pass.command_list_index_prev_function = 0; // TODO
      dst_pass.command_list_index_render_function = 0; // TODO
      dst_pass.command_list_index_post_function = 0; // TODO
  ...
  for (uint32_t i = 0; i < 2 * buffer_num; i++) {
    ...
    auto parsed_render_graph = ParseRenderGraph({{std::move(config)}});
    {
      ...
      allocators.back().push_back(std::move(command_allocators));
      {
        ...
        command_lists[pass.command_list_index_prev_function]->ResourceBarrier(1, &barrier);
        pass.render_function(&command_lists[pass.command_list_index_render_function], pass.resource_config);
        barrier.Transition.StateBefore = D3D12_RESOURCE_STATE_RENDER_TARGET;
        barrier.Transition.StateAfter  = D3D12_RESOURCE_STATE_PRESENT;
        command_lists[pass.command_list_index_post_function]->ResourceBarrier(1, &barrier);
      }
